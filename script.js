...
function bindOptionCards() {
  // Single-select cards (goal, activity)
  document.querySelectorAll('.opt[data-key]').forEach(function(el) {
    el.addEventListener('click', function() {
      var key   = el.dataset.key;
      var val   = el.dataset.val;
      var group = el.dataset.group;
      S[key] = val;
      document.querySelectorAll('#' + group + ' .opt').forEach(function(c) { c.classList.remove('on'); });
      el.classList.add('on');
    });
  });

  // Toggle cards (equipment)
  document.querySelectorAll('.opt.toggle').forEach(function(el) {
    el.addEventListener('click', function() {
      var key = el.dataset.key;
      if (S.equip.has(key)) {
        S.equip.delete(key);
...
function bindButtons() {
  document.getElementById('btn0').addEventListener('click',  function() { goTo(1); });
  document.getElementById('back1').addEventListener('click', function() { goTo(0); });
  document.getElementById('btn1').addEventListener('click',  function() { goTo(2); });
  document.getElementById('back2').addEventListener('click', function() { goTo(1); });
  document.getElementById('btn2').addEventListener('click',  function() { goTo(3); });
  document.getElementById('back3').addEventListener('click', function() { goTo(2); });
  document.getElementById('btn3').addEventListener('click',  function() { generate(); });
}

// ─── Day accordion ────────────────────────────────────────────────────────────

function bindDayToggle(container) {
  container.querySelectorAll('.day-hdr').forEach(function(hdr) {
...
async function fetchExerciseGif(searchTerm) {
  try {
    ...
    var res  = await fetch('https://oss.exercisedb.dev/api/v1/exercises/name/' + encoded + '?limit=3');
    var json = await res.json();
    if (json.data && json.data.length > 0) {
      for (var i = 0; i < json.data.length; i++) {
        if (json.data[i].gifUrl) return { gif: json.data[i].gifUrl, name: json.data[i].name };
      }
    }
  } catch (e) {}
  return null;
...
async function generate() {
  ...
  var act    = S.activity || 'moderate';
  var tDays  = parseInt(document.getElementById('tDays').value);
  var exp    = document.getElementById('exp').value;
  var eKey   = equipKey();

  // Nutrition
  var b    = Math.round(calcBMR(age, sex, height, weight));
  var tdee = Math.round(b * ACT[act]);
  var kcal = Math.round(tdee + GOAL_ADJ[goal]);
  var m    = MACROS[goal];
  var prot = Math.round((kcal * m.p) / 4);
  var carb = Math.round((kcal * m.c) / 4);
  var fat  = Math.round((kcal * m.f) / 9);
  var bmi  = (weight / Math.pow(height / 100, 2)).toFixed(1);

  var pool = shuffle((EXERCISE_MAP[goal][eKey] || EXERCISE_MAP[goal].bw).slice());
  ...
  var days   = (DAY_TYPES[goal] || DAY_TYPES.maintain).slice(0, tDays);
  var perDay = Math.min(5, Math.ceil(pool.length / tDays) + 1);
  var unique = pool.slice(0, Math.min(pool.length, tDays * perDay));

  // Fetch all GIFs in parallel
  document.getElementById('loader').querySelector('span').textContent = 'Fetching exercise demonstrations…';
  var gifResults = await Promise.all(unique.map(function(ex) { return fetchExerciseGif(ex.search); }));

  var TIPS = {
    lose:     'A 500 kcal/day deficit leads to ~0.5 kg/week of fat loss (Hall et al., 2012). Prioritize protein at 1.6–2.2 g/kg to preserve muscle during a cut.',
  ...
  for (var i = 0; i < days.length; i++) {
    ...
    for (var j = 0; j < perDay; j++) {
      var idx     = (i * perDay + j) % unique.length;
      var ex      = unique[idx];
      var gifData = gifResults[idx];
      var gifUrl  = gifData && gifData.gif  ? gifData.gif  : null;
      var exName  = gifData && gifData.name ? gifData.name : ex.search;
      var gifBlock = gifUrl
        ? '<div class="ex-gif"><img src="' + gifUrl + '" alt="' + exName + ' exercise demonstration" loading="lazy"><div class="gif-err" style="display:none;align-items:center;justify-content:center;width:100%;height:100%;"><i class="ti ti-barbell" style="font-size:28px;color:var(--text3)"></i></div></div>'
        : '<div class="ex-gif"><div class="gif-err" style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;"><i class="ti ti-barbell" style="font-size:28px;color:var(--text3)"></i></div></div>';
  ...
  planEl.innerHTML =
    '<div class="hero"><h2>Your personalized plan is ready</h2><p>Goal: ' + GOAL_LABEL[goal] + ' &nbsp;&middot;&nbsp; ' + tDays + ' days/week &nbsp;&middot;&nbsp; ' + exp + '</p></div>' +
    ...
    '<p style="font-size:13px;color:var(--text2);margin-bottom:1.25rem;">Tap any day to expand. Each exercise shows a real demonstration GIF and the equipment needed.</p>' +
    daysHTML +
    '<hr>' +
    '<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">' +
      '<button class="btn-sec" id="resetBtn"><i class="ti ti-refresh"></i>&nbsp; Start over</button>' +
  ...
  // Bind accordion on freshly rendered days
  bindDayToggle(planEl);
...
function resetApp() {
  ...
  loader.querySelector('span').textContent = 'Fetching real exercise demonstrations';
  document.querySelectorAll('.opt').forEach(function(c) { c.classList.remove('on'); });
  goTo(0);
}
